...
"""``instruction`` violated_support verifier."""
...
from uqlm.utils.prompts.violated_support.instruction import (
...
async def verify_instruction(
    ...
    **_: Any,
    ...
    """Evaluate every claim against the user's instruction inside ``input_text``."""
    if not claims:
        return []
    if llm is None:
        raise ValueError("verify_instruction requires a non-None LLM instance.")

    logger.info(
        "verify_instruction: dispatching %d concurrent LLM calls", len(claims)
    )
    tasks = [
...
async def _verify_single(
    ...
    *,
    ...
    messages = [
        SystemMessage(INSTRUCTION_SYSTEM_PROMPT),
        HumanMessage(prompt),
    ]
    try:
        ...
        logger.warning("verify_instruction: LLM call failed: %s", exc)
        raw_text = ""
